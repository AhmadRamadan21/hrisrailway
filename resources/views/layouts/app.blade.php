...
<head>
    ...
    <style>
        * {
        ...
        .sidebar .nav-item-toggle .chevron {
            transition: transform .2s ease;
            font-size: 12px;
        }

        .sidebar .nav-item-toggle[aria-expanded="true"] .chevron {
        ...
        @media (max-width: 576px) {
            ...
            /* Menyembunyikan teks nama user di HP agar tidak padat */
        }
    </style>
</head>
        ...
        <aside class="sidebar" id="mobileSidebar">
            <div class="sidebar-logo">
                <div class="sidebar-logo-brand">
                    <img src="{{ asset('assets/logo-ibp-icon.svg') }}" alt="Logo"
            ...
            <nav class="sidebar-nav">
                ...
                <div class="d-md-none p-3 mb-2 rounded-3 bg-light border">
                    <div class="d-flex align-items-center gap-2">
                        @if (auth()->user()->foto)
                            <img src="{{ asset(auth()->user()->foto) }}" class="rounded-circle object-fit-cover"
                        ...
                        <div class="overflow-hidden">
                            <div class="user-name text-truncate" style="font-size:13px;">{{ auth()->user()->name }}
                            </div>
                ...
                @if (auth()->user()->hasRole(['Admin', 'Super Admin']))
                    ...
                    <div class="collapse {{ request()->is('list-karyawan*', 'divisi-jabatan*', 'pengguna*') ? 'show' : '' }}"
                        id="dataKaryawanMenu">
                        <div class="nav-submenu">
                            <a href="{{ route('karyawan.index') }}"
                                class="nav-subitem {{ request()->is('list-karyawan*') ? 'active' : '' }}">
                                <i class="bi bi-person-lines-fill"></i> List Karyawan
                            </a>
                            <a href="{{ route('divisi-jabatan.index') }}"
                                class="nav-subitem {{ request()->is('divisi-jabatan*') ? 'active' : '' }}">
                                <i class="bi bi-diagram-3"></i> Divisi & Jabatan
                            </a>
                            <a href="{{ route('pengguna.index') }}"
                                class="nav-subitem {{ request()->is('pengguna*') ? 'active' : '' }}">
                                <i class="bi bi-person-badge"></i> Pengguna
                            </a>
                            <a href="{{ route('ai-knowledge.index') }}"
                                class="nav-subitem {{ request()->is('ai-knowledge*') ? 'active' : '' }}">
                ...
                <button class="nav-item nav-item-toggle {{ request()->is('absensi*', 'scan-qr*') ? 'active' : '' }}"
                    type="button" data-bs-toggle="collapse" data-bs-target="#absensiMenu"
                ...
                <div class="collapse {{ request()->is('absensi*', 'scan-qr*') ? 'show' : '' }}" id="absensiMenu">
                    <div class="nav-submenu">
                        <a href="{{ route('absensi.selfie') }}"
                            class="nav-subitem {{ request()->routeIs('absensi.selfie') ? 'active' : '' }}">
                            <i class="bi bi-camera"></i> Absen Selfie (500m)
                        </a>
                        <a href="{{ route('scan-qr.index') }}"
                            class="nav-subitem {{ request()->routeIs('scan-qr.index') ? 'active' : '' }}">
                            <i class="bi bi-qr-code-scan"></i> Scan QR Code
                        </a>
                        <a href="{{ route('absensi.index') }}"
                            class="nav-subitem {{ request()->routeIs('absensi.index') ? 'active' : '' }}">
                            <i class="bi bi-journal-check"></i> Riwayat Absensi
                        </a>
                    </div>
                </div>

                <a href="{{ route('kegiatan-harian.index') }}"
                    class="nav-item {{ request()->is('kegiatan-harian*') ? 'active' : '' }}">
                ...
                <a href="{{ route('penggajian.index') }}"
                    class="nav-item {{ request()->is('penggajian*') ? 'active' : '' }}">
                    <i class="bi bi-cash-coin"></i> Penggajian
                </a>

                <a href="{{ route('ai-chat.index') }}"
                    class="nav-item {{ request()->routeIs('ai-chat.index') ? 'active' : '' }}">
        ...
        <div class="main-area">
            <nav class="topbar">
                ...
                <div class="topbar-user">
                    ...
                    <div class="dropdown">
                        <a href="#"
                            class="d-flex align-items-center gap-2 text-decoration-none text-reset dropdown-toggle"
                            ...
                            <div class="user-details">
                                ...
                                <div class="text-muted" style="font-size: 12px; letter-spacing: .02em;">
                                    {{ auth()->user()->role->name ?? '-' }}</div>
                            </div>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end shadow-sm rounded-3 border-0 mt-2">
                            ...
                            <li>
                                <form method="POST" action="{{ url('/logout') }}">
                                    @csrf
                                    <button type="submit"
                                        class="dropdown-item text-danger py-2 px-3 d-flex align-items-center gap-2 fw-semibold">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('form').forEach(form => {
                form.addEventListener('submit', function(e) {
                    if (this.action.includes('/logout') || this.id === 'logout-form' || this.id ===
                        'aiChatForm' || this.method.toUpperCase() === 'GET') return;
                    e.preventDefault();
                    ...
                    Swal.fire(cfg).then(r => {
                        if (r.isConfirmed) {
                            Swal.fire({
                                title: isDelete ? 'Menghapus...' : 'Menyimpan...',
