    ...
    <div class="row g-2 align-items-center">
      ...
      <div class="col-12 col-md-4">
        <div class="input-group input-group-sm">
          ...
          <input type="text" id="filterSearch" class="form-control form-control-sm border-start-0 bg-light" placeholder="Cari nama, email, no. pegawai...">
        </div>
      </div>
      ...
      <div class="col-6 col-md-2 d-flex gap-1 justify-content-end">
        <button type="button" id="btnResetFilter" class="btn btn-sm btn-outline-secondary w-100 rounded-2" title="Reset filter">
          <i class="bi bi-arrow-clockwise"></i>
        </button>
        <a href="{{ route('karyawan.export.excel') }}" class="btn btn-sm btn-outline-success w-100 rounded-2" title="Export Excel">
          <i class="bi bi-file-earmark-excel"></i>
        </a>
        <a href="{{ route('karyawan.export.pdf') }}" target="_blank" class="btn btn-sm btn-outline-danger w-100 rounded-2" title="Export PDF">
      ...
      <table class="table table-hover align-middle mb-0" id="tableKaryawan" data-no-datatable>
        ...
        <tbody>
          @forelse($karyawans as $i => $k)
            <tr
              data-search="{{ strtolower($k->nama.' '.$k->email.' '.$k->no_pegawai) }}"
            ...
            >
              ...
              <td>
                <div class="avatar-circle" style="width: 36px; height: 36px; font-size: 12px; background: #2563eb; color: #fff; display: flex; align-items: center; justify-content: center; border-radius: 50%;">
              ...
              <td class="text-end pe-4 text-nowrap">
                <div class="d-flex justify-content-end gap-1">
                  <button class="btn btn-sm btn-outline-info rounded-2 px-2 py-1" data-bs-toggle="modal" data-bs-target="#modalLihatKaryawan{{ $k->id }}" title="Detail">
                    <i class="bi bi-eye"></i>
                  </button>
                  <button class="btn btn-sm btn-outline-primary rounded-2 px-2 py-1" data-bs-toggle="modal" data-bs-target="#modalEditKaryawan{{ $k->id }}" title="Edit">
                    <i class="bi bi-pencil"></i>
                  </button>
                  <form action="{{ route('karyawan.destroy', $k->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus karyawan ini?')">
                    @csrf
                    @method('DELETE')
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content border-0 shadow-lg rounded-4 overflow-hidden">
      ...
      <div class="modal-body p-4 pt-0">
        ...
        <div class="d-flex align-items-center gap-3 mb-4 p-3 rounded-4" style="background: linear-gradient(135deg, #eff6ff 0%, #e0e7ff 100%); border: 1px solid #dbeafe;">
          ...
          <div>
            ...
            <div class="d-flex align-items-center gap-2 mt-1">
              <span class="badge bg-white text-primary border border-primary-subtle px-2 py-1 rounded-pill fw-medium shadow-sm" style="font-size: 11px;">
                {{ $k->jabatan->nama_jabatan ?? 'Tanpa Jabatan' }}
              </span>
              <span class="badge {{ $k->status === 'Aktif' ? 'bg-success-subtle text-success border-success-subtle' : 'bg-secondary-subtle text-secondary border-secondary-subtle' }} border px-2 py-1 rounded-pill fw-medium shadow-sm" style="font-size: 11px;">
                <i class="bi {{ $k->status === 'Aktif' ? 'bi-check-circle-fill' : 'bi-x-circle-fill' }} me-1"></i>{{ $k->status }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="row g-3">
          <div class="col-6">
            <div class="p-3 rounded-3 h-100" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-upc-scan me-1"></i> No Pegawai</div>
              <div class="fw-semibold text-dark font-monospace">{{ $k->no_pegawai ?? '-' }}</div>
            </div>
          </div>
          <div class="col-6">
            <div class="p-3 rounded-3 h-100" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-diagram-3 me-1"></i> Divisi</div>
          ...
          <div class="col-12">
            <div class="p-3 rounded-3" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-envelope me-1"></i> Email</div>
              <div class="fw-semibold text-dark">{{ $k->email }}</div>
            </div>
          </div>
          <div class="col-6">
            <div class="p-3 rounded-3 h-100" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-telephone me-1"></i> No HP</div>
              <div class="fw-semibold text-dark">{{ $k->no_hp ?? '-' }}</div>
            </div>
          </div>
          <div class="col-6">
            <div class="p-3 rounded-3 h-100" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-calendar-event me-1"></i> Tanggal Lahir</div>
              <div class="fw-semibold text-dark">{{ $k->tanggal_lahir ? $k->tanggal_lahir->format('d M Y') : '-' }}</div>
            </div>
          </div>
          <div class="col-12">
            <div class="p-3 rounded-3" style="background-color: #f8fafc; border: 1px solid #f1f5f9;">
              <div class="small text-muted mb-1" style="font-size: 12px;"><i class="bi bi-geo-alt me-1"></i> Alamat</div>
              <div class="fw-medium text-dark lh-sm" style="font-size: 13.5px;">{{ $k->alamat ?? '-' }}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="modal-body p-4 pt-0">
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="text" name="nama" class="form-control rounded-3" value="{{ old('nama', $k->nama) }}" required>
            </div>

            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark small">No Pegawai</label>
              <input type="text" name="no_pegawai" class="form-control rounded-3" value="{{ old('no_pegawai', $k->no_pegawai) }}">
            </div>

            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark small">Email <span class="text-danger">*</span></label>
              <input type="email" name="email" class="form-control rounded-3" value="{{ old('email', $k->email) }}" required>
            </div>

            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark small">No HP</label>
              <input type="text" name="no_hp" class="form-control rounded-3" value="{{ old('no_hp', $k->no_hp) }}">
            </div>

            <div class="col-md-6">
              <label class="form-label fw-semibold text-dark small">Tanggal Lahir</label>
              <input type="date" name="tanggal_lahir" class="form-control rounded-3" value="{{ old('tanggal_lahir', optional($k->tanggal_lahir)->format('Y-m-d')) }}">
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="divisi_id" class="form-select rounded-3">
                ...
                @foreach($divisis as $d)
                  <option value="{{ $d->id }}" {{ old('divisi_id', $k->divisi_id) == $d->id ? 'selected' : '' }}>{{ $d->nama_divisi }}</option>
                @endforeach
              </select>
            </div>
            ...
            <div class="col-md-6">
              ...
              <select name="jabatan_id" class="form-select rounded-3">
                ...
                @foreach($jabatans as $j)
                  <option value="{{ $j->id }}" {{ old('jabatan_id', $k->jabatan_id) == $j->id ? 'selected' : '' }}>{{ $j->nama_jabatan }}</option>
                @endforeach
              </select>
            </div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function applyFilter() {
    ...
    rows.forEach(row => {
      ...
      row.style.display = (matchSearch && matchDivisi && matchJabatan && matchStatus) ? '' : 'none';
    });
  }
